...
\section{Add anc configure Keystone}
Follow instructions on
\usrref{https://docs.openstack.org/keystone/latest/install/index.html}
to install Pike keystone, then start the kerystone service.

Create the "swifty.owner" role with the
\begin{lstlisting}
openstack role create swifty.owner
\end{lstlisting}
command.

Then add "keystone": section to gate conf with url, user, pass and domain,
e.g. like this:
\begin{lstlisting}
keystone:
...
Gate uses the "/v3" API, no need to specify one in the URL string.
