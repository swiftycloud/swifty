...
def exclude_whitelist(dists):
    return set(dist for dist in dists if dist.project_name not in WHITELIST and dist.location.startswith("/packages"))
...
def remove_dist(dist):
    print("Will remove [%s] from [%s]" % (dist.project_name, dist.location))
    env = os.environ.copy()
    env["PYTHONPATH"] = dist.location
    subprocess.check_call(["pip", "uninstall", "-y", dist.project_name], env=env)
